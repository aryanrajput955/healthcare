...
export default function SignupPage() {
  ...
  const [formData, setFormData] = useState({
    name: "",
    email: "",
    organizationname: "",    // Matches backend
    organizationtype: "",    // Matches backend
    password: "",
    agreeToTerms: false,
  });
...
const handleSubmit = async (e) => {
  ...
  setIsLoading(true);
  try {
    // Payload now uses exact backend column names
    const payload = {
      name: formData.name.trim(),
      email: formData.email.trim(),
      password: formData.password,
      organizationname: formData.organizationname.trim(),     // Exact match
      organizationtype: formData.organizationtype.trim(),     // Exact match
    };
    ...
    if (data.access_token) {
      const userData = {
        name: payload.name,
        email: payload.email,
        organizationname: payload.organizationname,   // Changed from 'organization'
      };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#33A8D3] via-[#27A395] to-[#354B62] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div
        ref={containerRef}
        className="relative bg-white rounded-3xl shadow-2xl overflow-hidden max-w-6xl w-full grid lg:grid-cols-2"
      >
            ...
            <div className="space-y-6">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p className="text-white/80">
                    Boost your claims processing efficiency by up to 40%
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p className="text-white/80">
                    Full-featured content management tailored for healthcare
                  </p>
                </div>
              </div>
