...
const UserJourneyTimeline = () => {
  ...
  // -----------------------------------------------------------------------
  // 2. FETCH USER JOURNEYS – HANDLE 500 "No journeys found" AS EMPTY ARRAY
  // -----------------------------------------------------------------------
  useEffect(() => {
    ...
    const fetchUserJourneys = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://api.indiem.tech/user-journey/user/${user.id}`,
          {
            method: 'GET',
            headers: {
        ...
        // ---- SPECIAL CASE: backend returns 500 for empty result ----
        if (response.status === 500) {
          const raw = await response.text();
          if (raw.includes('No journeys found for this user')) {
            setUserJourneys([]);
            setLoading(false);
            return;
          }
  ...
  // -----------------------------------------------------------------------
  // 3. CREATE NEW JOURNEY – POST + REFRESH PAGE
  // -----------------------------------------------------------------------
  const createNewJourney = async (journeyCode) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to create journey: ${errText}`);
      }

      // Instead of updating state, refresh the page to show new journey
      window.location.reload();
    } catch (err) {
      console.error(err);
      alert(err.message || 'Could not create journey.');
    } finally {
      setCreating(false);
    }
  };
  ...
  // -----------------------------------------------------------------------
  // 4. PROCESS JOURNEYS (progress, steps, sorting, stats)
  // -----------------------------------------------------------------------
  const processedJourneys = useMemo(() => {
    const processed = userJourneys.map((uj) => {
      const formCount = uj.formResponses?.length || 0;
      const completedActions =
        uj.actionResponses?.filter((ar) => ar.response?.completed === true).length || 0;
  ...
  const stats = useMemo(() => {
    ...
    const averageProgress =
      total > 0 ? processedJourneys.reduce((s, j) => s + j.progress, 0) / total : 0;
  ...
  // -----------------------------------------------------------------------
  // 6. STATIC CASHLESS PROCESS STEPS
  // -----------------------------------------------------------------------
  const cashlessJourneySteps = [
    {
      step: 1,
      title: 'Medical Emergency/Planned Treatment',
      description: 'Patient experiences medical need...',
      timeline: 'Immediate',
      who: 'Patient',
      icon: AlertCircle,
      color: 'bg-[#27A395]',
    },
    {
      ...
      description: "Patient must choose from insurer's approved...",
      timeline: 'Immediate',
      who: 'Patient',
      icon: Shield,
      color: 'bg-[#33A8D3]',
    },
    {
      ...
      description: 'Patient admission process begins...',
    ...
    },
    {
      ...
      description: 'Formal notification to insurance company...',
    ...
    },
    {
      ...
      description: 'Hospital submits formal pre-authorization...',
    ...
    },
    {
      ...
      description: 'Hospital and patient submit required...',
      timeline: 'Along with pre-authorization...',
      who: 'Hospital and Patient',
      icon: FileText,
      color: 'bg-[#354B62]',
    },
    {
    ...
    },
    {
      ...
      description: 'Approval, rejection, or query...',
    ...
    },
    {
      ...
      description: 'Hospital medical team begins...',
      timeline: 'Immediately after approval',
      who: 'Hospital medical TCM',
      icon: User,
      color: 'bg-[#354B62]',
    },
    {
      ...
      description: 'Medical team completes treatment...',
    ...
    },
    {
      ...
      description: 'Comprehensive bill preparation...',
      timeline: 'At the time of discharge',
      who: 'Hospital billing',
      icon: FileText,
      color: 'bg-[#33A8D3]',
    },
    {
      ...
      description: 'Direct settlement between hospital...',
    ...
    },
    {
      ...
      description: 'Patient leaves hospital after settlement...',
  ...
  // -----------------------------------------------------------------------
  // 7. RENDER: LOADING, ERROR, MAIN UI
  // -----------------------------------------------------------------------
  if (loading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-6 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {/* SINGLE "Start New Journey" BUTTON – TOP ONLY */}
        <div className="flex justify-end mb-6">
          <div className="relative">
            ...
            >
              ...
              Start New Journey
              <ChevronDown
                className={`w-4 h-4 transition-transform ${showJourneyMenu ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {processedJourneys.length === 0 ? (
              ...
              <div className="text-center py-12 bg-white rounded-xl shadow-md border border-gray-100">
                ...
                <p className="text-sm text-gray-600">
                  You haven't started any claim journeys yet. Use the button above to begin.
                </p>
              </div>
            ) : (
              /* LIST OF JOURNEYS */
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 mb-3">
                            {journey.sortedSteps[0]?.title || 'Begin your claim process'}
                          </p>
                        ...
                        <div
                          className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ml-3 flex-shrink-0 ${
                          ...
                          {journey.sortedSteps.map((step) => (
                            <div
                              ...
                              onClick={() =>
                                step.isForm &&
                                !step.isCompleted &&
                                handleFormClick(journey.id, step.formId)
                              }
                            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/90 text-sm sm:text-base">
                    Complete step-by-step guide for hassle-free cashless claims
                  </p>
                </div>
              ...
              <div className="grid gap-4">
                {cashlessJourneySteps.map((step) => (
                  ...
                  >
                    <div className="flex-shrink-0">
                      <div
                        className={`w-12 h-12 rounded-lg ${step.color} flex items-center justify-center text-white shadow-md`}
                      >
